...
import { getDemoCourses, getDemoTasks, isMissingSupabaseTableError, setDemoTasks } from '../../lib/demoStore'
import { supabase } from '../../lib/supabase'
import { dateInputToTimestamp, formatDateOnly, timestampToDateInput } from '../../lib/dateOnly'
...
function toDateInput(value: string | null) { return timestampToDateInput(value) }
...
export function TasksPage({ initialInboxItem, onInboxProcessed }: TasksPageProps) {
  ...
  useEffect(() => {
    let isMounted = true
    if (!supabase) {
      const demoTasks = getDemoTasks<Task[]>([]).filter((task) => task.status === 'open')
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setIsSaving(true)
    setMessage('')
    const values = { title: draft.title.trim(), course_id: draft.course_id || null, due_at: dateInputToTimestamp(draft.due_at), effort_minutes: draft.effort_minutes ? Number(draft.effort_minutes) : null, importance: Number(draft.importance) }

    if (!supabase) {
      const nextImportance = Number(values.importance) as 1 | 2 | 3
      const savedTask: Task = editingTask ? { ...editingTask, ...values, importance: nextImportance, updated_at: new Date().toISOString() } : {
      ...
      const nextTasks = editingTask ? tasks.map((task) => task.id === editingTask.id ? savedTask : task) : [...tasks, savedTask]
      setTasks(nextTasks)
  ...
  function formatDue(value: string | null) {
    if (!value) return 'Belum ada deadline'
    const date = new Date(value)
    return date < new Date() ? `Terlewat · ${formatDateOnly(value)}` : `Deadline ${formatDateOnly(value)}`
  }
